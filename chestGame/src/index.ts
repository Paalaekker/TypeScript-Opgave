...
const arr: Number[] = [1,2,3,3,2,3,3,3,2,1];
let isclicked: boolean = false;
// Timers

const timeout: number = 2000;
const chestOpenTimeout: number = 1000;
const resetTimer: number = 8000;
...
function pickRandom(target: HTMLImageElement): boolean
{
    const r: Number = arr[Math.floor(Math.random() * arr.length)];
    if(r == +target.id)
    {
        return true;
    }
...
function getReward(target: HTMLImageElement): void {
...
function addImg(target: HTMLImageElement,imgName: String): void
{
...
function fadeOutEffect(target, ms: number) {
   
    var fadeEffect: NodeJS.Timer = setInterval(function () {
        
        if (!target.style.opacity) {
            target.style.opacity = '1';
        }
        if (target.style.opacity > '0') {
            let x:number = +textContainer.style.opacity - 0.1;
...
function gameOver(arg0: boolean) {
...
// 
function spawnReward(target: HTMLImageElement): void {
    setTimeout(() => {
        getReward(target);
...
function hide(target: Element, oldclass?: string): void
{
    if(oldclass)
...
function reveal(target: Element,newclass?: string): void
{
...
{
    const child:HTMLElement =  document.querySelector(".rewardImg");
    const element: HTMLImageElement = document.querySelector(".pickedChestImg");
    for(let i: number = 1;i<4;i++)
    {
        const element: HTMLElement = document.getElementById(`${i}`)
...
function changeImg(target:HTMLImageElement): void
{
    if(isclicked)
